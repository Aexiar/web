...
* 综上所述，在 JavaScript 中，函数是一种特殊的对象类型。它们拥有函数的行为和特性，并且可以像其他对象一样拥有自己的属性和方法。通过 typeof、instanceof、hasOwnProperty 和 toString 等方法，我们可以证明函数是对象。
...
> * 布尔类型的出现是为了处理逻辑判断和条件控制。在计算机程序中，经常需要进行条件判断，例如在 if 语句中根据某个条件来执行不同的代码块，或者在循环中判断是否继续迭代。这样的逻辑判断需要一个简单的、明确的数据类型，于是布尔类型应运而生。
...
* Java 中的自动装箱是指将基本数据类型自动转换为对应的包装类对象。例如：将 int 类型自动转换为 Integer 类型。而 JavaScript 中的自动装箱也是指将基本数据类型自动转换为对应的包装对象。例如：将 number 类型自动转换为 Number 对象。
