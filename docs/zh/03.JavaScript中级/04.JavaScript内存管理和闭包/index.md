...
>
> * ① 存储数据：内存可以临时存储计算机正在运行的程序和数据。当计算机执行程序时，需要将程序和数据加载到内存中，以便 CPU 能够快速访问和处理。内存的读写速度比硬盘和其他外部存储设备快得多，因此可以提高计算机的运行效率。
> * ② 运行程序：内存是计算机运行程序的主要场所。当用户打开一个应用程序或者操作系统启动时，相关的程序代码和数据会被加载到内存中，并由 CPU 进行处理。内存的容量和速度会影响计算机的性能和运行速度。
...
* `现代` 的编程语言都有自己的 `垃圾回收机制`。
  * 垃圾回收（是 Garbage Collection），简称 GC。
...
![卡通手绘寓言故事乌鸦喝水元素 PNG 图片素材下载_手绘 PNG_熊猫办公](./assets/3.jpeg)
