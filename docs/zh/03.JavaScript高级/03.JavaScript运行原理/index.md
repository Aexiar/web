...
* 官方对 V8 引擎的定义：
  ...
  * 它实现了 `ECMAScript` 和 `WebAssembly`，并在 Windows 7 或更高版本，macOS 10.12+ 和使用 x64，IA-32，ARM 或 MIPS 处理 器的 Linux 系统上运行。
...
* 通过使用即时编译器，JavaScript 代码的执行速度可以大大提高，使得 J avaScript 能够处理更复杂的任务和更大的数据量。许多现代的 JavaScript 引擎，如：V8 引擎和 SpiderMonkey 引擎，都使用了即时编译技术来提高 JavaScript 代码的执行效率。
